...
def expand_dataset(original_df, target_records=2000):
    ...
    for i in range(new_records_needed):
        ...
        # Create new record with proper data types
        new_record = {
            'Transaction ID': int(last_transaction_id + i + 1),
            'Date': date,
            ...
            'Price per Unit': int(price_per_unit),
            'Total Amount': int(quantity * price_per_unit)
        }
...
def main():
    ...
    validate_dataset_structure(original_df)
    ...
    validate_dataset_structure(expanded_df)
