...
class DemandPredictionChart extends ApexChartWidget
{
    ...
    protected function getFormSchema(): array
    {
        return [
            Select::make('timeFrame')
                ...
                ->helperText('Choose the prediction range to display')
                ->columnSpanFull()
                ->extraAttributes(['style' => 'max-width: 250px; margin-bottom: 16px; font-size: 1.1em; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;'])
        ];
    }
    ...
    protected function getOptions(): array
    {
        ...
        $today = date('Y-m-d');
        $nowMonth = date('Y-m');
        // Filter results to only future dates/months
        if ($this->timeFrame === '30_days') {
            $results = $results->filter(function($row) use ($today) {
                return $row->prediction_date >= $today;
            });
        } elseif ($this->timeFrame === '12_months') {
            $results = $results->filter(function($row) use ($nowMonth) {
                return substr($row->prediction_date, 0, 7) >= $nowMonth;
            });
        }
        ...
        foreach ($categories as $category) {
            ...
            if ($this->timeFrame === '12_months') {
                foreach ($dates as $ym) {
                    // Find all rows for this category and month
                    $monthRows = $results->where('shirt_category', $category)
                        ->filter(function($row) use ($ym) {
                            return strpos($row->prediction_date, $ym) === 0;
                        });
                    $sum = $monthRows->sum('predicted_quantity');
                    $data[] = $sum !== null ? (float)$sum : 0;
                }
            } else {
                foreach ($dates as $date) {
                    $row = $results->where('shirt_category', $category)->where('prediction_date', $date)->first();
                    $data[] = ($row && isset($row->predicted_quantity)) ? (float) $row->predicted_quantity : 0;
                }
            }
            // Ensure $data count matches $dates count
            while (count($data) < count($dates)) {
                $data[] = 0;
            }
            $series[] = [
                'name' => $category,
