...
class PlaceOrder extends Page
{
    ...
    public function placeOrder($id)
    {
        if (isset($this->cart[$id])) {
            ...
            // Debug: Log the current delivery options
            \Log::info('Delivery options check', [
                'cart_item_id' => $id,
                'delivery_options' => $this->delivery_options,
                'specific_option' => $this->delivery_options[$id] ?? 'NOT_SET'
            ]);

            // Check if delivery option is selected
            if (!isset($this->delivery_options[$id]) || empty($this->delivery_options[$id]) || $this->delivery_options[$id] === null) {
                $this->dispatch('cart-updated', [
