...
class PlaceOrder extends Page
{
    ...
    public function calculateItemTotal($id)
    {
        ...
        number_format( $product->unit_price - ($product->unit_price * 0.05)) 
        if ($packages['starter'] > 0) {
            $totalDiscountPercent += 2; // 2% for starter
        }
        if ($packages['standard'] > 0) {
            $totalDiscountPercent += 3; // 3% for standard (classic)
        }
        if ($packages['premium'] > 0) {
            $totalDiscountPercent += 5; // 5% for premium
        }
