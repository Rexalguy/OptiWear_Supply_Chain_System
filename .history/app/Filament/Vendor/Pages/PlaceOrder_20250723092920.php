...
class PlaceOrder extends Page
{
    ...
    public function mount()
    {
        ...
        foreach ($this->cart as $id => $item) {
            ...
            if (!isset($this->cart[$id]['delivery_option'])) {
                $this->cart[$id]['delivery_option'] = 'pickup'; // Default to pickup
            }
        }
        ...
        // Sync delivery options with cart items
        foreach ($this->cart as $id => $item) {
            if (!isset($this->delivery_options[$id])) {
                $this->delivery_options[$id] = $item['delivery_option'] ?? 'pickup';
            }
        }
    }
    ...
    public function placeFullOrder()
    {
        if (empty($this->cart)) {
            $this->dispatch('cart-updated', [
                'title' => "Cart is empty. Please add items before placing order.",
                'icon' => 'warning',
            ]);
            return;
        }
        ...
        // Create separate orders for different delivery options or one order with mixed items
        // For now, we'll create one order and store delivery option per item
        $order = VendorOrder::create([
            'created_by' => Auth::id(),
            'status' => 'pending',
            'total' => $total,
            'delivery_option' => 'mixed', // Indicates multiple delivery options
            'order_date' => now(),
            'expected_fulfillment' => now()->addDays(5),
        ]);

        // Create order items with individual delivery options
        foreach ($this->cart as $item) {
            VendorOrderItem::create([
                ...
                'delivery_option' => $item['delivery_option'] ?? 'pickup', // Store delivery option per item
            ]);
        }

        // Clear cart and delivery options
        $this->cart = [];
        $this->cartCount = 0;
        $this->delivery_options = [];
        session()->put('cart', []);
        session()->put('cartCount', 0);
        session()->forget('delivery_options');
        
        $this->dispatch('cart-updated', [
            'title' => "Order placed successfully! Order ID: {$order->id}",
            'icon' => 'success',
        ]);
    }
}