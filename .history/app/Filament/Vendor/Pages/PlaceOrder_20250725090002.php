...
class PlaceOrder extends Page
{
    ...
    public $cart;
    public $cartCount;
    public $totalPrice = 0; // Total price of the cart
    ...
    public function mount()
    {
        ...
        foreach ($this->cart as $id => $item) {
            ...
            $item['price'] = $item['price'] ?? 0; // Default to 0 if not set
            $item['price'] *= 0.95; // Apply 5% discount
            $this->cart[$id]['price'] = $item['price'];
    ...
    public function calculateItemTotal($id)
    {
        ...
        if ($packages['starter'] > 0) {
            $this->totalDiscountPercent += 2; // 2% for starter
        }
        if ($packages['standard'] > 0) {
            $this->totalDiscountPercent += 3; // 3% for standard (classic)
        }
        if ($packages['premium'] > 0) {
            $this->totalDiscountPercent += 5; // 5% for premium
        }

        // Apply discount
        $discountAmount = $baseTotal * ($this->totalDiscountPercent / 100);
