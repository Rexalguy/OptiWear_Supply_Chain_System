    ...
    protected function getOptions(): array
    {
        ...
        foreach ($categories as $category) {
            ...
            foreach ($dates as $date) {
                ...
                $data[] = ($row && isset($row->predicted_demand)) ? (float) $row->predicted_demand : null;
            }
            $series[] = [
                'name' => $category,
                'data' => $data,
        ...
        // Summaries (ignore rows without predicted_demand)
        $validResults = $results->filter(fn($r) => isset($r->predicted_demand));
        $max = $validResults->sortByDesc('predicted_demand')->first();
        $min = $validResults->sortBy('predicted_demand')->first();
        $total = $validResults->sum('predicted_demand');
