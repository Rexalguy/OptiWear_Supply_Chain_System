...
#visualization: Bar graph for comparison of actual vs predicted
import matplotlib.pyplot as plt

#reset index to align Series
y_test_reset = y_test.reset_index(drop=True)
y_pred_series = pd.Series(y_pred, name="Predicted Quantity")

#ombine into a DataFrame for easier plotting
comparison_df = pd.DataFrame({
    "Actual Quantity": y_test_reset,
    "Predicted Quantity": y_pred_series
})

# Plot bar chart for first 20 predictions
