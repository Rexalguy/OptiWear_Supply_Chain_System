<x-filament-panels::page>
    ...
    <div class="p-6 space-y-6">
        ...
        {{-- Cart token info --}}
        <div
            class="token-cart-wrapper flex flex-col md:flex-row justify-between items-center mb-10 p-4 bg-gray-50 dark:bg-gray-800 rounded-md shadow-sm">
            <div class="token-message text-base md:text-lg text-gray-800 dark:text-gray-200 mb-3 md:mb-0 font-medium">
                @if ($potentialTokens > 0)
                    🎁 You will earn <strong class="text-primary-600 dark:text-primary-400">{{ $potentialTokens }}</strong>
                    token{{ $potentialTokens > 1 ? 's' : '' }} for this order!
                @else
                    Make a purchase above <strong class="text-primary-600 dark:text-primary-400">UGX 50,000</strong> to earn
                    tokens 🎁
                @endif
            </div>

            <a href="{{ url('/customer/my-orders') }}"
                class="relative inline-flex items-center gap-2 px-5 py-3 text-base font-semibold text-white bg-gradient-to-r from-sky-500 to-sky-700 rounded-md transition hover:from-sky-600 hover:to-sky-800 custom-cart-btn">
                <x-heroicon-o-shopping-cart class="w-5 h-5" />
                View Cart

                @if ($this->cartCount > 0)
                    <span
                        class="cart-badge absolute top-0 right-0 -mt-2 -mr-2 flex items-center justify-center w-5 h-5 text-xs font-bold text-white bg-red-600 rounded-full">
                        {{ $this->cartCount }}
                    </span>
                @endif
            </a>
        ...
        @else
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                @foreach ($products as $product)
                    <div
                        class="product-card dark-gradient-card rounded-lg overflow-hidden {{ $selectedCategory === 'recommendations' ? 'recommendations-glow' : '' }}">
                        ...
                        <div class="mt-4 px-4 pb-4">
                            <x-filament::button size="sm" color="primary" class="custom-order-btn full-width-animated-btn"
                                wire:click="openProductModal({{ $product->id }})" icon="heroicon-o-shopping-cart">
                                Order Now
                            </x-filament::button>
                        </div>
                    </div>
