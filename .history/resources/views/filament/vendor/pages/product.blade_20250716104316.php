<x-filament-panels::page>
    <div class="carousel-container">
        <div class="carousel" wire:ignore>
            @foreach ($products as $product)
                <div class="carousel-item">
                    <img src="{{ $product->image ? asset('storage/' . $product->image) : '/images/image.png' }}" alt="Carousel Image" class="w-full h-auto object-cover rounded-md">
                </div>
            @endforeach
        </div>
        <div class="carousel-controls">
            <button class="prev-btn" wire:click="prevImage">Previous</button>
            <button class="next-btn" wire:click="nextImage">Next</button>
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        let interval = null;
        window.livewireCarouselAuto = function() {
            if (interval) clearInterval(interval);
            interval = setInterval(function() {
                window.Livewire.emit('nextImage');
            }, 3000); // Change image every 3 seconds
        };
        window.livewireCarouselAuto();
    });
</script>
