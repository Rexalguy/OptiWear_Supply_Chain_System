    ...
    @forelse ($cart as $item)
        ...
        <div style="color: white !important; background-color: rgb(53, 53, 53) !important;" class="flex gap-6 p-4  rounded-lg mb-4 shadow-sm">
            ...
            <div class="w-80 bg-gray-50 p-4 rounded-lg">
                <h2 style="color: black !important;" class="font-semibold text-lg mb-3">Package Breakdown</h2>
                @if(isset($item['packages']['premium']) && $item['packages']['premium'] > 0)
                    <div class="mb-2">
                        <li class="text-sm text-gray-600">Premium Packages: {{ $item['packages']['premium'] }}</li>
                    </div>
                @endif
                @if(isset($item['packages']['standard']) && $item['packages']['standard'] > 0)
                    <div class="mb-2">
                        <li class="text-sm text-gray-600">Classic Packages: {{ $item['packages']['standard'] }}</li>
                    </div>
                @endif
                @if(isset($item['packages']['starter']) && $item['packages']['starter'] > 0)
                    <div class="mb-2">
                        <li class="text-sm text-gray-600">Starter Packages: {{ $item['packages']['starter'] }} (2% Disco)</li>
                    </div>
                @endif
            </div>
        </div>
