    ...
    <div class="rounded border-gray-200 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-6">
        ...
        >
            <div wire:click="openProductModal({{ $product->id }})" class="w-full h-40 flex items-center justify-center bg-gray-100 rounded-md mb-2 overflow-hidden">
                <img src="{{ $product->image ? asset('storage/' . $product->image) : '/images/image.png' }}" alt="{{ $product->name }}s" class="h-full w-auto object-contain">
            </div>
            <div>
                <div>
            ...
            <div class="shadow bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md relative modal-slide-up" style="cursor:auto;" @click.stop>
                ...
                <div>
                    <div>
                        ...
                        <p class="text-sm ">Unit Price: UGX {{ number_format($clickedProduct->price) }}</p>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (window.Livewire && livewireEl) {
            window.Livewire.find(livewireEl.getAttribute('wire:id'))?.call('syncCart', cart, cartCount);
            console.log('Cart :', cart, 'Cart Count:', cartCount);
        } else {
            console.warn('Livewire not found. Cart synchronization may not work.');
        }
    });
    window.addEventListener('beforeunload', function () {
        const cart = @json(session('cart', []));
        const cartCount = @json(session('cartCount', 0));
