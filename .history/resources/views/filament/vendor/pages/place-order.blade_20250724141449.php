...
<div>
    ...
    @forelse ($cart as $item)
        ...
        <div style="color: white !important; background-color: #4b5563; border-radius: 2rem !important;" class="flex gap-6 p-4 rounded-[2rem] mb-4 shadow-sm">
            {{-- Left side: Product details and controls --}}
            <div class="flex-1">
                {{-- Product image --}}
                <div style="background-color: white !important;" class="w-48 h-48 flex items-start justify-center bg-white rounded-[1.5rem] mb-3 overflow-hidden flex-shrink-0">
                    <img src="{{ asset($item['image'] ?? 'images/default-product.png') }}" alt="{{ $item['name'] ?? 'Product' }}" class="w-auto h-auto object-cover rounded-[1.5rem] max-w-none min-w-0 min-h-0" style="width: 280px !important; height: auto !important;">
                </div>
            ...
            <div class="w-80 p-4 rounded-[1.5rem] bg-slate-400">
