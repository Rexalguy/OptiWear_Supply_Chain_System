...
<x-filament-panels::page>
    ...
    $filteredRows = $segmentResults;
    if ($segmentFilter) {
        $filteredRows = $filteredRows->where('segment_label', $segmentFilter);
    }
    @endphp
        ...
        <div style="display: flex; flex-wrap: wrap; gap: 2rem; align-items: flex-start;">
            ...
            <div style="flex: 2 1 400px; min-width: 320px;">
                <form method="get" style="margin-bottom: 1rem;">
                    <label for="segment" class="font-semibold mr-2">Filter by Segment:</label>
                    <select name="segment" id="segment" onchange="this.form.submit()" style="padding: 0.5rem 1rem; border-radius: 0.5rem; border: 1px solid #ccc; min-width: 180px;">
                        <option value="">All Segments</option>
                        @foreach($allSegments as $seg)
                            <option value="{{ $seg }}" @if($segmentFilter == $seg) selected @endif>{{ $seg }}</option>
                        @endforeach
                    </select>
                </form>
                <table style="width:100%; background: #f9fafb; border-radius: 0.5rem; overflow: hidden; border-collapse: collapse;">
                    <thead>
                        <tr style="background: #f3f4f6;">
                            ...
                            <th style="padding: 0.5rem 1rem; text-align: left;">Gender</th>
                            <th style="padding: 0.5rem 1rem; text-align: left;">Age Group</th>
                            <th style="padding: 0.5rem 1rem; text-align: left;">Shirt Category</th>
                    ...
                    <tbody>
                        @foreach($filteredRows as $row)
                            <tr>
                                <td style="padding: 0.5rem 1rem;">{{ $row->segment_label }}</td>
                                <td style="padding: 0.5rem 1rem;">{{ $row->gender }}</td>
                                <td style="padding: 0.5rem 1rem;">{{ $row->age_group }}</td>
