...
<x-filament-panels::page>
    <div class="bg-white rounded-xl shadow p-8 max-w-4xl mx-auto mb-8 relative">
        <h2 class="text-2xl font-bold mb-6">Demand Prediction (Line Chart)</h2>
        <form method="get" class="absolute top-8 right-8 mb-0 z-10">
            ...
            <select name="time_frame" id="time_frame" onchange="this.form.submit()"
                class="px-4 py-2 rounded-lg border border-gray-300 min-w-[180px]">
        ...
        <script>
            // --- DEMAND PREDICTION LINE CHART (Reverted to original style) ---
            var options = {
                chart: {
                    type: 'line',
                    height: 500,
        ...
        @endphp
        <div class="bg-gray-50 rounded-lg p-6 max-w-3xl mx-auto mt-8 mb-8">
                ...
                <table class="min-w-full bg-white rounded-xl">
                    <thead>
                        <tr class="bg-gray-100">
                            <th class="px-6 py-3 text-left font-bold text-gray-700">Category</th>
                            <th class="px-6 py-3 text-right font-bold text-gray-700">Total Predicted Demand</th>
                            <th class="px-6 py-3 text-right font-bold text-gray-700">Share (%)</th>
                        </tr>
                    </thead>
                    <tbody>
                        @php
            ...
            @endphp
            <div class="overflow-x-auto rounded-xl shadow mb-8">
                <table class="min-w-full bg-white rounded-xl mb-4">
                    <thead>
                        <tr class="bg-gray-100">
                            <th class="px-6 py-3 text-left font-bold text-gray-700">Category</th>
                            <th class="px-6 py-3 text-left font-bold text-gray-700">Trend</th>
                            <th class="px-6 py-3 text-right font-bold text-gray-700">% Change (First → Last)</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($trendRows as $row)
                            <tr class="hover:bg-blue-50 even:bg-gray-50 transition">
                                <td class="px-6 py-3">{{ $row['category'] }}</td>
                                <td class="px-6 py-3">{{ $row['trend'] }}</td>
                                <td class="px-6 py-3 text-right">{{ $row['pct'] }}%</td>
                            </tr>
                        @endforeach
                        <tr class="font-bold bg-gray-50">
                            <td class="px-6 py-3">Total</td>
                            <td class="px-6 py-3">{{ $totalTrend }}</td>
                            <td class="px-6 py-3 text-right">
    ...
    <div class="bg-white rounded-xl shadow p-8 max-w-4xl mx-auto mt-8">
        <h2 class="text-2xl font-bold mb-6">Customer Segmentation</h2>
            ...
            <div class="flex-2 min-w-[320px]">
                ...
                <div class="overflow-x-auto rounded-xl shadow mb-8">
                    <table class="min-w-full bg-white rounded-xl">
                        <thead>
                            <tr class="bg-gray-100">
                                <th class="px-6 py-3 text-left font-bold text-gray-700">Segment</th>
                                <th class="px-6 py-3 text-left font-bold text-gray-700">Top Shirt Category</th>
                                <th class="px-6 py-3 text-right font-bold text-gray-700">Total Purchased</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($segmentTopBuys as $row)
                                <tr class="hover:bg-blue-50 even:bg-gray-50 transition">
                                    <td class="px-6 py-3">{{ $row->segment }}</td>
                                    <td class="px-6 py-3">{{ $row->shirt_category }}</td>
                                    <td class="px-6 py-3 text-right">{{ $row->total_purchased }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
